    ...
    public partial class OrientedMusic
    {
        ...
        public static IEstimator<ITransformer> BuildPipeline(MLContext mlContext)
        {
            ...
            var pipeline = mlContext.Transforms.ReplaceMissingValues(new []{new InputOutputColumnPair(@"Father's Highest Education Level", @"Father's Highest Education Level"),new InputOutputColumnPair(@"Mother's Highest Education Level", @"Mother's Highest Education Level"),new InputOutputColumnPair(@"Average Previous Marks", @"Average Previous Marks"),new InputOutputColumnPair(@"Study Hours", @"Study Hours")})      
                                    ...
                                    .Append(mlContext.MulticlassClassification.Trainers.OneVersusAll(binaryEstimator:mlContext.BinaryClassification.Trainers.FastTree(new FastTreeBinaryTrainer.Options(){NumberOfLeaves=4,MinimumExampleCountPerLeaf=12,NumberOfTrees=139,MaximumBinCountPerFeature=34,FeatureFraction=0.685189354449207,LearningRate=0.00204060464091519,LabelColumnName=@"Actual Result",FeatureColumnName=@"Features"}),labelColumnName: @"Actual Result"))      
